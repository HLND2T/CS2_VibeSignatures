...
- The normalized configuration is passed to `preprocess_func_xrefs_via_mcp` as the unified func-xref fallback

## Contract
- The old tuple schema is no longer supported; any match is treated as invalid configuration immediately
- `xref_strings`, `xref_gvs`, `xref_signatures`, and `xref_funcs` are positive candidate sources; multiple positive sources are intersected
...
- Explicit address literals are currently recognized by the `0x` prefix; example: `xref_gvs: ["0x1805407C8"]`
...
## Related memories
- `preprocess_common_skill_func_xrefs`: newer summary of the same configuration entry
- `preprocess_func_xrefs_via_mcp`: underlying xref resolution, candidate-set intersection, and exclusion logic
