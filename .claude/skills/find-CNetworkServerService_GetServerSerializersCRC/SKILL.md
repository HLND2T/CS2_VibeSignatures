...
description: |
  Final-guarantee fallback for the find-CNetworkServerService_GetServerSerializersCRC preprocessor.
  Recovers CNetworkServerService::GetServerSerializersCRC (a virtual function of the CNetworkServerService
  ...
  (ida_preprocessor_scripts/find-CNetworkServerService_GetServerSerializersCRC.py) could not resolve the target.
  Trigger: CNetworkServerService_GetServerSerializersCRC
disable-model-invocation: true
---

# Find CNetworkServerService_GetServerSerializersCRC (final-guarantee fallback)

Recover the virtual function `CNetworkServerService::GetServerSerializersCRC` in CS2 `engine2.dll` /
`libengine2.so` using IDA Pro MCP tools. This is the **Agent fallback** for the
`find-CNetworkServerService_GetServerSerializersCRC` skill: it runs only when the preprocessor script
...
call site (`; 0x148/0x150 = ... = CNetworkServerService_GetServerSerializersCRC`). Treat their addresses as
...
`CNetworkServerService::GetServerSerializersCRC(this)` is an indirect virtual call on the global
...
1. From the resolved `vfunc_offset`, read the vtable slot of `CNetworkServerService_vtable` to obtain the
   concrete function address, and rename it to `CNetworkServerService_GetServerSerializersCRC` with
...
3. **ALWAYS** Use SKILL `/write-vfunc-as-yaml` with:
   - `func_name`: `CNetworkServerService_GetServerSerializersCRC`
...
- Windows (`engine2.dll`): `CNetworkServerService_GetServerSerializersCRC.windows.yaml`
- Linux (`libengine2.so`): `CNetworkServerService_GetServerSerializersCRC.linux.yaml`
