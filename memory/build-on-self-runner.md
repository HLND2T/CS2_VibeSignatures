...
## Overview
`.github/workflows/build-on-self-runner.yml` validates an exact release source, requires a published warm IDB cache generation, builds immutable symbol/gamedata candidates, stages the complete private bin tree, and creates the generated-output PR. The PR merge remains the promotion gate for accepted release state, but warm-cache publication is available earlier and independently.
## Responsibilities
...
- Copy accepted persisted analysis state without IDA database files, then restore binaries and `.i64` only from the returned generation.
...
- Stage the complete analyzed private bin tree plus manifests, then create an immutable generated-output PR.
...
## Architecture
Preflight -> required reusable warm-cache producer -> exact generation restore -> strict analysis -> immutable symbol and gamedata validation -> complete private-bin staging -> generated-output PR. After that PR merges, [[promote-release-after-output-merge]] verifies the staged inventory and transactionally replaces accepted `PERSISTED_WORKSPACE/bin/<GAMEVER>`.
## Dependencies
...
- `stage-build` still copies the complete analyzed workspace `bin/<GAMEVER>` into `release-staging`; `.i64` is not excluded and is bound into the private inventory.
- When the generated-output PR merges, `promote-bin` transactionally swaps that staged tree into accepted persisted bin. Therefore the historical release-to-persisted `.i64` path was indirect, not absent.
- Accepted persisted `.i64` remains release state but is not a supported warm-cache input for subsequent builds.
