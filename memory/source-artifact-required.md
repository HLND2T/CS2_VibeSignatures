...
- [fact] Since the base-inherited-selected bridge (branch `dev-base-inherited-selected`, 2026-09-06) the plan schema is **4** and carries a top-level `execution_strategy` (`fresh-full-v1` default | `base-inherited-selected-v1`), sourced from the base-owned `source_artifact_policy.yaml` optional key `execution_strategy`. Each maintained GAMEVER report now partitions outputs into `execute_groups` / `execute_nodes` / `inherit_paths` / `inherited_absent_groups` / `removed_paths` (old `affected_producer_groups` / `selected_alternative_nodes` names removed; consumers updated: verifier, `new_gamever_artifact.py` bootstrap check, tests). The execution closure adds same-session prerequisite back-propagation. `pr-self-runner.yml` routes its execute step on `preparation.execution_strategy`: selected runs `ida_analyze_bin.py -selected_execution <manifest>` against a seeded actual root; fresh-full keeps `-force_all` with an empty root. Selected is **not yet enabled** (policy defaults to fresh-full); enabling is a single policy-file bridge after Phase-D real-runner comparison. Selected evidence cannot impersonate full: independent report domain `source2-selected-execution:v1`, and `release_artifact_rebuild.py` still demands force-all v2.