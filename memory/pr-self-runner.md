...
- 只从 producer 返回的 explicit generation 恢复 configured binaries 与 `.i64`，并校验 cache key、inventory 与 consumer IDA kernel version。
...
4. accepted persisted bin 若存在，按 `.stignore` 加仓库内硬编码 IDA suffix 排除复制非 IDB 基线；不存在时由 cache generation 提供 binaries。
5. consumer 验证本机 IDA kernel version 后，`idb_cache.py restore` 从 explicit generation 恢复 configured binaries 与 `.i64`；缺失、篡改或 identity 不匹配立即失败。
