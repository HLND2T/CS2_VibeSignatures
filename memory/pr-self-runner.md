...
- [fact] Policy activation after Phase-D (2026-09-07) selects `base-inherited-selected-v1`; `mode=full`/the `validate-full` job name still denote the self-hosted route, not a force-all command. The worker explicitly sets `CS2VIBE_STRING_MIN_LENGTH=4` as used in the real comparison. Since PR #950 (2026-09-08) the value is defined once at workflow-level `env:` and referenced in the job via `${{ env.CS2VIBE_STRING_MIN_LENGTH }}`, and the same definition was added to `build-on-self-runner.yml` (release build), `bootstrap-new-gamever-artifacts.yml`, and `source-artifact-full-bridge.yml` so every win64 producer session pins minlen=4 explicitly instead of relying on runner-local `.env` or code defaults (code default also happens to be 4 via `DEFAULT_IDA_STRING_MIN_LENGTH`, but bootstrap/release artifacts must byte-match the PR-validation settings).
...
- Local test environment caveat: a `.env` value `CS2VIBE_STRING_MIN_LENGTH=4` is loaded during aggregate discovery and breaks two existing IDA mocks (`test_collect_xref_func_starts_for_string_uses_substring_by_default`, `test_py_eval_extracts_standard_and_extended_registration_calls`); the same failures reproduce on unchanged base `44f28161` with that environment value. For deterministic unit/aggregate checks, set this variable to an empty string inside the test Python process before discovery (PowerShell 5.1 empty environment assignment removes it and lets dotenv reload 4). This is a test-only override; preserve the runner's production value 4.
