#!/usr/bin/env python3
"""Preprocess script for find-CGamePlayerEquip_API_TriggerForAllPlayers skill."""

from ida_preprocessor_scripts._define_inputfunc import (
    API_INPUT_HANDLER_PTR_OFFSET,
    preprocess_define_inputfunc_skill,
)

TARGET_NAME = "CGamePlayerEquip_API_TriggerForAllPlayers"
INPUT_NAME = "CGamePlayerEquip_API::TriggerForAllPlayers"
HANDLER_PTR_OFFSET = API_INPUT_HANDLER_PTR_OFFSET
ALLOWED_SEGMENT_NAMES = (".data",)
RENAME_TO = "CGamePlayerEquip_API_TriggerForAllPlayers"

GENERATE_YAML_DESIRED_FIELDS = [
    # (symbol_name, generate_yaml_fields)
    (
        "CGamePlayerEquip_API_TriggerForAllPlayers",
        [
            "func_name",
            "func_va",
            "func_rva",
            "func_size",
            "func_sig",
        ],
    ),
]


async def preprocess_skill(
    session,
    skill_name,
    expected_outputs,
    old_yaml_map,
    new_binary_dir,
    platform,
    image_base,
    debug=False,
):
    """Locate the CGamePlayerEquip_API::TriggerForAllPlayers input handler from its _API input descriptor."""
    return await preprocess_define_inputfunc_skill(
        session=session,
        expected_outputs=expected_outputs,
        platform=platform,
        image_base=image_base,
        target_name=TARGET_NAME,
        input_name=INPUT_NAME,
        generate_yaml_desired_fields=GENERATE_YAML_DESIRED_FIELDS,
        handler_ptr_offset=HANDLER_PTR_OFFSET,
        allowed_segment_names=ALLOWED_SEGMENT_NAMES,
        rename_to=RENAME_TO,
        debug=debug,
    )
